...
router.get('/', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const learningTargets = await prisma.learningTarget.findMany({
      where: { userId },
...
router.post('/', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const learningTarget = await prisma.learningTarget.create({
      data: {
        userId,
        skillName,
...
router.put('/:id/complete', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const target = await prisma.learningTarget.findUnique({ where: { id } });
        if (!target) {
        ...
        const updatedTarget = await prisma.learningTarget.update({
            where: { id },
