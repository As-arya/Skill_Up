...
router.post('/validate-cv', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { userId, cvText, cvImage, mimeType } = req.body;
...
router.get('/job-suggestions', requireAuth, async (_req: AuthRequest, res: Response): Promise<void> => {
  res.status(200).json({ jobs: KNOWN_JOB_TITLES });
});

router.post('/validate-job', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const fuzzyMatch = KNOWN_JOB_TITLES.find(j => 
      j.toLowerCase().includes(normalized) || normalized.includes(j.toLowerCase())
    );
    if (fuzzyMatch) {
  ...
  try {
    ...
    if (result.skillGaps && Array.isArray(result.skillGaps)) {
      for (const gap of result.skillGaps) {
        ...
        for (const subSkill of subSkills) {
          const existing = await prisma.skill.findFirst({ where: { userId, name: subSkill } });
          if (!existing) {
            ...
            await prisma.skill.update({ where: { id: existing.id }, data: { category: groupName } });
          }
        }
      }
    }
    ...
    if (result.masteredSkills && Array.isArray(result.masteredSkills)) {
      for (const skillName of result.masteredSkills) {
        const existing = await prisma.skill.findFirst({ where: { userId, name: skillName } });
        if (!existing) {
          await prisma.skill.create({
            data: { userId, name: skillName, category: 'Mastered', isChecked: true }
          });
        } else if (!existing.isChecked) {
          await prisma.skill.update({ where: { id: existing.id }, data: { isChecked: true } });
        }
      }
    }
