...
router.get('/', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const [user, totalSkills, acquiredSkills, projects, target, skillsList] = await Promise.all([
      ...
      prisma.skill.count({ where: { userId, isChecked: true } }),
      prisma.project.count({ where: { userId } }),
      prisma.learningTarget.findFirst({ where: { userId }, orderBy: { createdAt: 'desc' } }),
      prisma.skill.findMany({ where: { userId, isChecked: true }, take: 5, orderBy: { createdAt: 'desc' } })
    ]);
    ...
    if (targetRoleName) {
      ...
      if (jobReadiness === 0) jobReadiness = 15; // Give baseline progress
