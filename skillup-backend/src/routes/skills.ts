...
router.get('/', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const groupMap = new Map<string, { skills: typeof skills; mastered: number }>();
    for (const skill of skills) {
      const group = skill.category || 'General';
      if (!groupMap.has(group)) groupMap.set(group, { skills: [], mastered: 0 });
...
router.post('/confirm-mastery', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    for (const name of skillNames) {
      const skill = await prisma.skill.findFirst({ where: { userId, name } });
      if (skill && !skill.isChecked) {
        await prisma.skill.update({ where: { id: skill.id }, data: { isChecked: true } });
        updated++;
      }
    }
...
router.post('/cleanup', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const allSkills = await prisma.skill.findMany({ where: { userId } });
    const corruptedIds = allSkills
      .filter(s => s.name.startsWith('{') && s.name.includes('group:'))
      .map(s => s.id);

    if (corruptedIds.length > 0) {
      await prisma.skill.deleteMany({ where: { id: { in: corruptedIds } } });
    }
